...
if AnalysisType =='Bulk':
    ColumnNames = EnergyVilleLoadbyZone.columns.tolist() + RenewProfiles.columns.tolist() # Extracting column names
...
AnnualTSeries= pd.concat([EnergyVilleLoadbyZone,RenewProfiles], axis=1) # Single matrix, colnames = ColumnNames
...
# Normalised for clustering, then denormalised again
...
def GetKMedoidsClusteringOutputs (InputData,NumGrpDays,NClusters, PeakDayinCluster, LoadWeight):
    ...
    for j in range(len(OldColNames)):
        ...
        if OldColNames[j] in LoadColNames: 
            ...
            # Load is weighted twice as heavily as renewables for the clustering process
...
################################# pre-processing data to create concatenated column of load, pv and wind data
    ...
    NumDataPoints =round(Nhoursperyear/24/NumGrpDays) # Number of clusters

    DataPointAsColumns = ['p' + str(j) for j in range(1,NumDataPoints+1)] # Number of representations within each cluster

    # Create a dictionary storing groups of time periods to average over for each hour
    HourlyGroupings = {i:[j for j in range(NumGrpDays*24*(i-1),NumGrpDays*24*i)] for i in range(1,NumDataPoints +1)} # Constituents of each cluster, splitting to weeks
    
    #  Create a new dataframe storing aggregated load and renewables time series
    ModifiedDataNormalized = pd.DataFrame(columns = DataPointAsColumns)
    # Original data organized in concatenated column
    ModifiedData = pd.DataFrame(columns = DataPointAsColumns) # Single column

    # Creating the dataframe with concatenated columns
    for j in range(0,NumDataPoints): # 52 This loop is creating the 52-column matrix 
...
################################## k-means clustering process
    # create Kmedoids clustering model and specify the number of clusters gathered
    ...
    if PeakDayinCluster =='yes': # If peak day in cluster, generate one less cluster
        NClusters = NClusters -1 # If a peak day will be manually added later, remove this column from the clustering input
    
    # K-means clutering with 100 trials with randomly selected starting values
    model = KMedoids(n_clusters=NClusters, init ='k-medoids++')  
    ...
    # Number of points in kth cluster (i.e. label=0)
        ...
        # Compute Euclidean distance of each point from centroid of cluster k, finding point closest to centroid
...
# Selecting rows corresponding to Load in excluded subperiods and exclude them from scale factor calculation
    NRowsLoad = len(LoadColNames) # Just the peak value
 # Excluding grouping with peak hr from scale factor calculation
 # Could be selecting peak day within each cluster?
    if PeakDayinCluster =='yes': 
...
# Add the grouping with the peak hour back into the cluster if that is excluded in the clustering
    ...
    # This is just plotting the load duration curve, order wont be chronological, its in a descending order
...
#        print(EachClusterWeight[j])
        if EachClusterWeight[j] >1: # Need to duplicate entries only weight is greater than 1
            # Creating a full year of data by duplicating the clusters according to weight 
    ...
    -np.sort(FullLengthOutputs[OldColNames[j]].values)) for j in range(len(OldColNames))} # Sorting in ascending/descending order, so the orders don't matter
    print(RMSE)
...
#        print(PossibleNClusters[j])
        ...
        for k in range(len(SelectPeakDayinCluster)):
            ...
            = GetKMedoidsClusteringOutputs(AnnualTSeries,PossibleNumGrpDays[i],PossibleNClusters[j],
...
for k in range(3):  # three load profiles
 ...
 plt.suptitle('RMS Error for Load Profiles, K-Medoids', fontsize = 'x-small', y = 0.92)
 plt.savefig('kmed_LoadError_'+'MaxGrpDays_' + str(PossibleNumGrpDays[-1]) + 'MaxNumClusters_' + str(PossibleNClusters[-1]) +'Peakhr_'+ str(ForcePeakday) +'.png',bbox_inches="tight")

# solar pv profile
fontsizeval = 7
sns.set(font_scale=1.1, context ='talk',style ='darkgrid')
plt.figure(figsize=(6,10))
for k in range(3,6):  # four pv profiles
 plt.subplot(5,4,k-2)
 for i in range(len(PossibleNumGrpDays)):
     plt.plot(PossibleNClusters,[OutputsbyGrpsbyNClusters[PossibleNumGrpDays[i]][PossibleNClusters[j]][ForcePeakday]['RMSE'][ColumnNames[k]] for j in range(len(PossibleNClusters))],label=str(PossibleNumGrpDays[i]) +'d')
     plt.title(ColumnNames[k] , fontsize=fontsizeval, fontweight='bold')
 ...
 plt.tick_params(axis='y',labelsize =fontsizeval)
 plt.tick_params(axis='x',labelsize =fontsizeval)
 plt.suptitle('RMS Error for Solar, K-Medoids')
 plt.savefig('pverror_'+'maxgrpdays_' + str(PossibleNumGrpDays[-1]) + 'maxnumclusters_' + str(PossibleNClusters[-1]) +'peakhr_'+'.png',bbox_inches="tight")
##
### Wind profile
##fontsizeval = 7
##sns.set(font_scale=1.1, context ='talk',style ='darkgrid')
...
for i in range(3):
 ...
 plt.tick_params(axis='y',labelsize =fontsizeval)
 plt.tick_params(axis='x',labelsize =fontsizeval)
plt.savefig('kmed_Load_DurationCurve_'+'GrpDays_' + str(NumGrpDays) + 'NumClusters_' + str(NClusters) +'Peakhr_'+ str(ForcePeakday) +'.png',bbox_inches="tight")
...
for i in range(2):
 ...
 plt.tick_params(axis='y',labelsize =fontsizeval)
 plt.tick_params(axis='x',labelsize =fontsizeval)
plt.savefig('kmed_'+'SolarCFCurve_'+'GrpDays_' + str(NumGrpDays) + 'NumClusters_' + str(NClusters) +'Peakhr_'+ str(ForcePeakday) +'.png',bbox_inches="tight")
...
ClusteringOutput = pd.read_csv('kmed_' + 'AZ_' +
                   'NumGrpDays_'+ str(NumGrpDays) +'_NClusters_' + str(NClusters) +
...
LoadSeries.to_csv('kmed_Load_data.csv',index=False)
